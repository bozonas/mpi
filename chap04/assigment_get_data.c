...
main(int argc, char **argv)
{
    ...
    float Trap(float local_a, float local_b, int local_n, int f,
    ...
    integral = Trap(local_a, local_b, local_n, f, h);
    ...
    int p /* in  */)
{
    ...
    if (my_rank == 0)
    {
        printf("Enter a, b, n and function nr:\n1 - f(x)=x^2\n2 - f(x)=x^2 + x\n3 - f(x)=x^3\n");
        ...
        for (dest = 1; dest < p; dest++)
        {
            ...
            MPI_Send(n_ptr, 1, MPI_INT, dest, tag,
                     MPI_COMM_WORLD);
            tag = 3;
            MPI_Send(f_ptr, 1, MPI_INT, dest, tag,
                     MPI_COMM_WORLD);
        }
    }
    else
    {
        ...
        MPI_Recv(n_ptr, 1, MPI_INT, source, tag,
                 MPI_COMM_WORLD, &status);
        tag = 3;
        MPI_Recv(f_ptr, 1, MPI_INT, source, tag,
                 MPI_COMM_WORLD, &status);
    }
} /* Get_data */
